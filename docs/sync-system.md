...
- 当前版本使用快照覆盖，不做细粒度合并
- `GardenState.waterBaby` 保存木头/石头、已完成修缮项目和当前单次探险；它随农场快照同步，不新增独立 D1 表
- 为兼容尚未认识 `waterBaby` 的旧客户端，Worker 接收不含该字段的新快照时会保留云端已有的 `waterBaby`；显式携带该字段的新客户端仍按正常 LWW 覆盖

### 4.4 设置快照

#### 表：`settings_snapshots`
...
- `snapshot_json` 对应 `SettingsSnapshot`
- 当前只同步跨设备应该一致的设置项，例如每日目标、杯量、杯量步进、提醒间隔、久坐提醒间隔、活跃时间段、语言
...
#### `POST /api/sync/settings/push`

用途：

- 把当前本地设置快照上传到 `settings_snapshots`
...
- 组合接口是当前前端优先使用的同步接口
- 单独的日快照、农场快照、设置快照接口仍然保留，用于兼容旧客户端或调试
...
4. 通过 `/api/sync/snapshots` 拉取云端近期状态
5. 通过 `apply_remote_snapshots` 和 `apply_remote_settings_snapshot` 按规则应用到本地
...
顶部刷新按钮会执行同样的近期快照拉取流程。

### 8.4 用户切换到今日或历史页时

当用户切换到 `today` 或 `history` 页签时，前端会自动拉取一次近期状态。

这一步的目标是：

- 用户在查看高频数据页面时，尽量看到另一个设备刚刚产生的近期变化
- 不要求用户理解“饮水”“农场”“设置”是不同数据域
...
用户在设置页保存设置时，会上传当前设置快照到 `settings_snapshots`。

当前设置快照同步的是跨设备应该一致的账户级设置，不包括当前设备本地属性。

### 8.7 手动同步设置时

用户点击“同步设置”时，会拉取云端设置快照。

如果云端设置快照比本地设置快照更新，则覆盖本地对应设置；否则保留本地设置。
...
对于用户手动触发的同步，界面会显示错误信息。

对于本地写入后的后台同步：

- 本地写入已经完成，不会回滚用户刚刚记录的数据
- 如果 3 次尝试后仍然失败，会提示“已保存到本地，云端同步暂时没有完成”
- 用户可以稍后点击“同步近期状态”重试
...
- 当前版本农场同步采用完整快照覆盖
- 不做更细粒度的库存级合并
- 水宝宝的开始探险、领取物资和完成修缮分别通过 `start_expedition`、`claim_expedition`、`build_garden_project` 修改同一农场快照
...
- 否则保留本地
- 唯一的字段级兼容例外是：较新的上传快照完全缺少 `waterBaby` 时，服务端保留现有云端 `waterBaby`，避免旧桌面端或小程序抹掉新养成状态

### 10.3 设置快照覆盖规则

- 如果云端设置快照 `updated_at` 晚于本地设置快照时间，则用云端覆盖本地对应设置
- 否则保留本地设置
...
首次启动时，应用会先展示一个简短说明，而不是直接弹出补记或提醒相关提示。
...
用户点击“开始使用”后，本地会记录 `syncMeta.onboardingSeenAt`，后续启动不再重复展示。

如果用户想了解更完整的同步机制，首启说明中提供“了解同步”展开说明；设置页也保留了更详细的同步帮助入口。

---

## 14. 饮水快照与农场快照是否应该合并同步

从用户视角看，饮水记录和农场状态都属于高频数据，用户不应该需要理解它们是两套快照，也不应该需要分别点击两个同步动作。

因此可以把它们在“交互层”和“接口层”合并，但不建议把它们在“存储层”和“冲突版本”上合并成同一个实体。

### 14.1 当前实现

当前实现已经在多数用户路径上把二者一起拉取：

- 应用启动时会一起拉取日快照和农场快照
- 配对成功后会一起拉取日快照和农场快照
- 手动拉取/顶部刷新会一起拉取日快照和农场快照
- 切换到今日或历史页时会一起拉取日快照和农场快照
- 本地写入后的后台同步会通过组合接口上传本次变更内容：饮水操作只带日快照，农场操作只带农场快照，设置保存只带设置快照

也就是说，用户已经不需要分开操作“饮水同步”和“农场同步”。
...
- `GET /api/sync/snapshots` 同时返回最近 7 天日快照、当前农场快照、当前设置快照
- `POST /api/sync/snapshots/push` 接收可选的 `dailySnapshots`、`gardenSnapshot`、`settingsSnapshot`
- 客户端本地仍然调用一次“同步近期状态”，不用关心内部拆成哪些数据域
...
不建议把饮水快照、农场快照、设置快照都塞进同一张大表或同一个全局版本号。

原因是：

- 日快照是按天分片的，农场和设置是账户级单快照，数据粒度不同
- 饮水、农场、设置的写入频率和冲突风险不同
- 如果共用一个全局 `updated_at`，一次农场操作可能会让较旧的饮水快照看起来“整体更新”，增加误覆盖风险
- 分开保存可以独立做 Last-Write-Wins，也方便以后把某一类数据升级成事件流或更细粒度合并

推荐方向是：

- 合并 API 请求
- 合并用户操作入口
- 保持 D1 表和本地版本元数据按数据域拆分

这样既能降低用户理解成本，也能保留当前同步模型的可维护性。
...
- 本地保存完整状态
- 云端用 `D1` 保存最近 7 天的日快照、当前农场快照与当前设置快照
- 云端用 `R2` 保存完整 JSON 备份
