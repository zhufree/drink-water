...
import { MemberGardenModal } from "./leaderboardPanel/MemberGardenModal";
...
export function LeaderboardPanel({
  ...
  const nicknameStatusClass =
    nicknameSaveState === "success"
      ...
      : nicknameSaveState === "error"
        ? "text-amber-200/88"
        : "text-slate-400/70";
  const selectedMemberCanRemove = Boolean(
    selectedMember && isOwner && selectedMember.accountId !== viewerAccountId
  );
  ...
  return (
    <section className="flex flex-col gap-3">
              ...
              {leaderboard.length > 0 ? (
                <div className="flex flex-col gap-2">
                  {leaderboard.map((entry) => (
                    <article
                      key={entry.accountId}
                      onClick={() => void handleOpenMemberGarden(entry)}
                      className="flex cursor-pointer items-center justify-between gap-3 rounded-[18px] bg-white/6 p-3 transition hover:-translate-y-px hover:bg-white/10"
                    >
      ...
      <MemberGardenModal
        ...
        loading={memberGardenLoading}
        error={memberGardenError}
        history={memberGarden?.history ?? []}
        garden={memberGarden?.garden ?? null}
        canRemove={selectedMemberCanRemove}
  ...
  async function handleOpenMemberGarden(entry: LeaderboardEntry) {
    setSelectedMember(entry);
    setMemberGarden(null);
    setMemberGardenError(null);
    if (!deviceId || !activeCircleCode) {
      setMemberGardenError(t("leaderboard.memberGardenError"));
      return;
    }

    setMemberGardenLoading(true);
    try {
      const result = await getCircleMemberGarden({
        deviceId,
        circleCode: activeCircleCode,
        targetAccountId: entry.accountId,
        rangeDays: 28
      });
      setMemberGarden(result);
    } catch (error) {
      setMemberGardenError(
        error instanceof Error ? error.message : t("leaderboard.memberGardenError")
      );
    } finally {
      setMemberGardenLoading(false);
    }
  }
